    ...
    protected function _sendProposalEmail($customerId, $realQuoteadvId, $quoteId)
    {
        try {
            ...
            if (empty($res)) {
                ...
                Mage::helper('qquoteadv')->sentAnonymousData('proposal', 'b', $quoteId);
            }
        } catch (Exception $e) {
            $message = $this->__("Qquote proposal email was't sent to the client for quote #%s", $realQuoteadvId);
            ...
            try {
                if (isset($data['product']) && is_array($data['product']) && count($data['product']) > 0) {
                    ...
                    if ($quoteId = (int)$this->getRequest()->getParam('id')) {
                                    ...
                                    if (count($errors)) {
                                        ...
                                        foreach ($errors as $message) {
                                            if ($message != $lastMessage) {
                                                $message .= __('<br />Quote could not be saved.');
                                                Mage::getSingleton('adminhtml/session')->addError($message);
                                            }
                                            $lastMessage = $message;
                                        }
                        ...
                        } elseif (Mage::getStoreConfig('qquoteadv/quote_advanced/checkout_alternative', $_quoteadv->getData('store_id')) > 0) {
                            $_quoteadv->setData('alt_checkout', 0);
                        }

                        // Salesrule
                        if ($salesrule = $this->getRequest()->getParam('salesrule')) {
								$oCoupon = Mage::getModel('salesrule/coupon')->load($salesrule, 'code');
								if ($oCoupon->getRuleId())
								{
                            		$_quoteadv->setSalesrule($oCoupon->getRuleId());
								}
								else
								{
	                                Mage::getSingleton('adminhtml/session')->addError($this->__('"%s" is not a valid coupon code.', $salesrule));
								}
                        } else {
...
//                                $_quoteadv->save();
                        ...
                        if ($this->getRequest()->getParam('back')) {
                            ...
                            Mage::helper('qquoteadv')->sentAnonymousData('save', 'b', $quoteId);

                            // Check for negative profit
                            if (Mage::getStoreConfig('qquoteadv/general/profit', $_quoteadv->getData('store_id')) != 1) {
                            ...
                            } elseif ($hold == 2) // Set Quote to 'Unhold'
                            {
                                ...
                                if ($statusses) {
                                    ...
                                    if (!$statusses->getSubstatus()) {
                                        // If no substatus was set
                                        // clear substatus data
                                        $_quoteadv->setSubstatus();
                                    }
                                } else {
    ...
    public function deleteAction()
    {
        ...
        if ($id > 0) {
            try {
                ...
                Mage::helper('qquoteadv')->sentAnonymousData('cancel', 'b', $id);
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
    ...
    public function massUpdateAction()
    {
        ...
        if (Mage::helper('qquoteadv')->validLicense('mass_update_quote_requests')) {
            ...
            $message = $this->__('This function is only available in the enterprise edition. <a href="http://www.cart2quote.com/magento-quotation-module-pricing.html">Upgrade</a>');
    ...
    public function sendEmail($params)
    {
        ...
        if (Mage::getStoreConfig('qquoteadv/quote_advanced/checkout_alternative', $_quoteadv->getData('store_id')) && $_quoteadv->getData('alt_checkout')) {
            $quoteadv_param = Mage::getStoreConfig('qquoteadv/quote_advanced/checkout_alternative_email', $_quoteadv->getStoreId());
        }
...
<div >';
    ...
    public function convertAction()
    {
        ...
        if (!empty($quoteadvId)) {
			...
			//set customer name/email
			$this->_getOrderCreateModel()->getQuote()->setCustomerFirstname($customer->getFirstname());
			$this->_getOrderCreateModel()->getQuote()->setCustomerLastname($customer->getLastname());
			$this->_getOrderCreateModel()->getQuote()->setCustomerEmail($customer->getEmail());
            ...
            Mage::helper('qquoteadv')->sentAnonymousData('confirm', 'b', $quoteadvId);
        ...
        if ($customerId && $email) {
            ...
            foreach ($quote->getAllVisibleItems() as $item) {
                ...
                if ($item->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
                    ...
                    //$final_price = $item->getProduct()->getFinalPrice(); //this is wholesale price? 
                    if ($item->getProduct()->getSpecialPrice())
					{
						$original_price = $item->getProduct()->getSpecialPrice(); //gives sale price
					}
					else 
					{
            ...
            try {
                ...
                // Update Address data
                // TODO:
                // create a better way to store all this data
                ...
                Mage::helper('qquoteadv')->sentAnonymousData('request', 'b', $modelCustomer->getData('quote_id'));
            } catch (Exception $e) {
                Mage::log($e->getMessage());
            }
