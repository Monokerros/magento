...
//        }
    ...
    public function getPdf($quotes = array())
    {
        ...
        $this->columns = array(
            array(Mage::helper('qquoteadv')->__('Product image'), $this->_leftTextPad, $this->y, 'UTF-8'),
            array(Mage::helper('qquoteadv')->__('Product name'), 100, $this->y, 'UTF-8'),
    ...
    {
        $showPrice = ($this->_quoteadv->getData('itemprice') == 1) ? true : false;
        $line = array();
        $drawItems = array();
        $attr = array();
        ...
        if ($item->getTypeId() == 'bundle') {
            ...
            // Loading virtual product to get right product info, like sku
            $virtualProd = Mage::helper('qquoteadv')->getQuoteItem($item, $unit->getAttribute());
        }
    ...
    protected function insertTitles(&$page, $source, $storeId)
    {
        ...
        $page->setLineWidth(0.5);
        $page->drawRectangle($this->_leftRectPad, $y, 570, $this->y -= 55);
...
//            $expDays    = (int) date_diff(date_create($proposalDate), date_create($expiryDate))->format('%a');
    ...
    public function insertShippingMethods()
    {
        ...
        // TODO:
        // Add message if no shipping methods are available
    ...
    protected function insertTotal(&$page)
    {
        ...
        // To default totals
        if (Mage::getStoreConfig('qquoteadv/general/adjustment', $store) == 1) {
